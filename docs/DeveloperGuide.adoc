...
[NOTE] If the index given is not a valid index (ie, out of bounds or negative), it will return an error to the user rather than attempting to close the Eatery.
...
[NOTE] If the command is missing a `Name`, `Address`, or `Category` parameter, the execution of the add command will fail and the application will throw a `ParseException`.
...
|`* * *` |user |find an eatery by name |locate details of eateries without having to go through the entire list
