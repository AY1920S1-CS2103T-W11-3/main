...
Step 3. The address book now returns a success message upon successfully adding a review to the eatery.

[NOTE]
If an eatery is `closed`, the user will be shown an error message saying that the eatery is closed.
...
When the user attempts to edit review, the eatery is accessed using
