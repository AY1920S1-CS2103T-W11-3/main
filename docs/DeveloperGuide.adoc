...
* `EateryList#hasEatery()` -- Checks if an Eatery with the same identity as Eatery eatery exists in the address book.
* `EateryList#addEatery()` -- Adds an Eatery eatery to the address book. The eatery must not already exist in the address book.
...
Step 2. `MainWindow#showStats()` will then if `StatsWindow` is already open, if yes then it will call `StatsWindow#initStats()` and focuses the window. Else, it will call `StatsWindow#initStats()` and shows the window.
